...
with open(res_file, 'r') as fh:
    for buf in fh:
        ...
        orf     = fields[5].replace(';', '')
        vgene   = fields[1].replace(';', '')
        dgene   = fields[2].replace(';', '')
        jgene   = fields[3].replace(';', '')
        cdr3    = fields[4].replace(';', '')
        #print [seq_id, vgene, vgene, jgene, cdr3]
        if orf == 'has_stop_codon':
            orf = '*'
        else:
            orf = 'N'
